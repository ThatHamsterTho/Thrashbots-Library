#include "Controller.h"
#include "NHDLCD.h"

/*
Controller C;

void setup() {
  // put your setup code here, to run once:
  Serial.begin(9600);
  //Serial.println("hello world");

  C.begin();

  C.SetDemux(CSN::LCD);
  digitalWrite(REGS, LOW);
  digitalWrite(NRST, HIGH);
}

void loop() {
  // put your main code here, to run repeatedly:
  Serial.print(C.GetJoystick(Controller::Left,  Joystick::H)); Serial.print(","); Serial.print(C.GetJoystick(Controller::Left,  Joystick::V)); Serial.print(",");
  Serial.print(C.GetJoystick(Controller::Right, Joystick::H)); Serial.print(","); Serial.print(C.GetJoystick(Controller::Right, Joystick::V)); Serial.print(",");
  Serial.print(C.GetPot(Controller::Left)); Serial.print(","); Serial.print(C.GetPot(Controller::Right)); Serial.print(",");
  Serial.print(C.GetBumper(Controller::Left) * 1024); Serial.print(","); Serial.print(C.GetBumper(Controller::Right) * 1024); Serial.print(",");
  Serial.print(C.GetSwitch(Controller::Left) * 1024); Serial.print(","); Serial.println(C.GetSwitch(Controller::Right) * 1024);
}

*/

/****************************************************
*               Pinout on Arduino Uno               *
*****************************************************/

/****************************************************
*               Hex Table for Image Pic             *
*****************************************************/

GLCD glcd;

unsigned char NHD_Logo [] = {

};

unsigned char FMRadio [] = {

};


/*****************************************************
*           Setup Function, to run once              *
*****************************************************/

void setup() {  
  InitControllerPins();

  Serial.begin(9600);
  glcd.init();   
}

/*****************************************************
*           Loop Function, to run repeatedly         *
*****************************************************/

void loop() {
delay(10);   
  while(1)
  {
    glcd.DispCharSet();  
  }
}
